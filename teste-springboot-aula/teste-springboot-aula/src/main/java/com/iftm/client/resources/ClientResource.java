...
public class ClientResource {
	...
	@GetMapping(value = "/findByIncome/{income}")
	public ResponseEntity<Page<ClientDTO>> findByIncome(@PathVariable Double income,
																								@RequestParam(value = "page", defaultValue = "0") Integer page,
																								@RequestParam(value = "linesPerPage", defaultValue = "12") Integer linesPerPage,
																								@RequestParam(value = "direction", defaultValue = "ASC") String direction,
																								@RequestParam(value = "orderBy", defaultValue = "name") String orderBy) {
		PageRequest pageRequest = PageRequest.of(page, linesPerPage, Direction.valueOf(direction), orderBy);
		Page<ClientDTO> byIncome = service.findByIncome(pageRequest, income);
		return ResponseEntity.ok().body(byIncome);
	}
